...
<head>
    ...
    <title>Checkout - {{ $plan->name }} - Adoring Fans</title>
            ...
            theme: {
                extend: {
                    colors: {
                        ...
                        green: { // Adicionado para o status de pago
...
<body class="bg-gray-100 dark:bg-gray-900 min-h-screen text-gray-900 dark:text-gray-100 transition-colors duration-200">
    <div class="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <form id="checkout-form" class="grid lg:grid-cols-3 gap-8"> {{-- Removido action/method para AJAX --}}
            @csrf
    ...
    <script>
        ...
        // Função para mostrar notificações (assumindo que está em app.blade.php)
        ...
        // Limpa mensagens de erro por campo
        ...
        // Exibe mensagens de erro por campo
        function displayFieldErrors(errors) {
            clearFieldErrors(); // Limpa antes de exibir novos
            for (const field in errors) {
                const errorElement = document.querySelector(`[data-error="${field}"]`);
                if (errorElement) {
                    errorElement.textContent = errors[field][0]; // Mostra apenas o primeiro erro
        ...
        document.querySelector('input[name="phone"]').addEventListener('input', function(e) {
            ...
            if (value.length > 0) {
                ...
                value = value.length > 7 ? value + '-' + value.substring(7, 11) : value;
            }
        ...
        // Animação nos campos de input (foco)
        document.querySelectorAll('input').forEach(input => {
            ...
            input.addEventListener('blur', function() {
                this.classList.remove('ring-2', 'ring-pink-500', 'border-pink-500');
            });
        });

        // Lógica de submissão do formulário via AJAX
        checkoutForm.addEventListener('submit', async function(e) {
            e.preventDefault(); // Previne a submissão padrão do formulário

            clearFieldErrors(); // Limpa erros anteriores
            ...
            try {
                const response = await fetch("{{ route('checkout.store', $plan->hash_id) }}", {
                    method: 'POST',
                    headers: {
        ...
        async function verificarPagamento(hash) {
            ...
            try {
                ...
                if (response.ok && result.status !== 'paid') {
                    ...
                    `;
                    paymentStatusBox.classList.add('paid');
                    
                    showNotification('Pagamento confirmado! Redirecionando...', 'success');
                    setTimeout(() => {
                        // Redireciona para uma página de sucesso final, passando o hash da transação
                        window.location.href = "{{ route('checkout.success', '') }}/" + hash; 
                    }, 3000);
        ...
        // Exibir mensagens de sessão (success, error, info) ao carregar a página
