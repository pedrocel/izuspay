...
@section('title', 'Usuários - Administração')
...
<div class="space-y-6">
    ...
    <div class="bg-gradient-to-r from-gray-50 to-white dark:from-gray-800 dark:to-gray-800 rounded-xl p-6 border border-gray-200 dark:border-gray-700 shadow-lg">
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-gray-600 dark:text-gray-400">Gerencie todos os usuários do sistema de forma eficiente</p>
            </div>
            <a href="#" 
               class="inline-flex items-center space-x-2 bg-purple-600 hover:bg-purple-700 text-white px-6 py-3 rounded-lg font-medium transition-all duration-200 hover:transform hover:scale-105 shadow-lg hover:shadow-xl">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-200 dark:border-gray-700 hover:shadow-lg transition-shadow">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-green-600">{{ $users->where('status', 'ativo')->count() }}</p>
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-200 dark:border-gray-700 hover:shadow-lg transition-shadow">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-yellow-600">{{ $users->where('status', 'pendente')->count() }}</p>
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-200 dark:border-gray-700 hover:shadow-lg transition-shadow">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-red-600">{{ $users->where('status', 'inativo')->count() }}</p>
                </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-200 dark:border-gray-700">
        <div class="flex flex-col lg:flex-row gap-4">
            ...
            <div class="flex gap-2">
                <select class="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
    ...
    <div class="space-y-4">
            ...
            <div class="p-6">
                <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                    ...
                    <div class="flex items-center space-x-4">
                        <div class="relative">
                            @if($user->avatar_url)
                                <img class="w-14 h-14 rounded-full object-cover border-2 border-purple-200 dark:border-purple-700" 
                                     src="{{ $user->avatar_url }}" 
                                     alt="{{ $user->name }}">
                            @else
                                <div class="w-14 h-14 bg-gradient-to-r from-purple-400 to-pink-400 rounded-full flex items-center justify-center">
                            ...
                            <div class="absolute -bottom-1 -right-1 w-5 h-5 rounded-full border-2 border-white dark:border-gray-800 
                                        {{ $user->status == 'ativo' ? 'bg-green-500' : ($user->status == 'pendente' ? 'bg-yellow-500' : 'bg-red-500') }}">
                            </div>
                        </div>
                        <div class="flex-1 min-w-0">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white truncate">{{ $user->name }}</h3>
                            <p class="text-sm text-gray-600 dark:text-gray-400 flex items-center space-x-1">
                                <i data-lucide="mail" class="w-4 h-4"></i>
                                <span class="truncate">{{ $user->email }}</span>
                            </p>
                            <div class="flex items-center space-x-4 mt-2">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                    {{ $user->perfilAtual() ? 'bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-400' : 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-400' }}">
                                    <i data-lucide="shield" class="w-3 h-3 mr-1"></i>
                                    {{ $user->perfilAtual() ? $user->perfilAtual()->name : 'Sem Perfil' }}
                                </span>
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                    {{ $user->status == 'ativo' ? 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400' : 
                                       ($user->status == 'pendente' ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-400' : 
                    ...
                    <div class="flex flex-col lg:flex-row lg:items-center gap-4">
                        <div class="text-sm text-gray-500 dark:text-gray-400">
                            <div class="flex items-center space-x-1">
                                ...
                                <span>{{ $user->ultimo_acesso ? $user->ultimo_acesso->diffForHumans() : 'Nunca acessou' }}</span>
                            </div>
                        </div>
                        
                        <div class="flex items-center space-x-2">
                            <a href="#" 
                               class="inline-flex items-center px-3 py-2 text-sm font-medium text-purple-600 hover:text-purple-700 dark:text-purple-400 dark:hover:text-purple-300 bg-purple-50 hover:bg-purple-100 dark:bg-purple-900/20 dark:hover:bg-purple-900/30 rounded-lg transition-colors">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-12 text-center">
            ...
            <p class="text-gray-500 dark:text-gray-400 mb-6">Comece criando seu primeiro usuário no sistema.</p>
            <a href="#" 
               class="inline-flex items-center space-x-2 bg-purple-600 hover:bg-purple-700 text-white px-6 py-3 rounded-lg font-medium transition-colors">
                <i data-lucide="user-plus" class="w-5 h-5"></i>
    ...
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-white dark:bg-gray-800 rounded-xl max-w-md w-full p-6 transform transition-all">
            ...
            <p class="text-gray-600 dark:text-gray-400 text-center mb-6">
                Tem certeza que deseja excluir o usuário <span id="deleteUserName" class="font-medium"></span>? 
...
<script>
    function confirmDelete(userId, userName) {
        ...
        // Ajuste a rota de exclusão conforme a sua rota real para usuários de administração
