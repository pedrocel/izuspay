...
@section('title', 'Banners - AssociaMe')
...
<div class="space-y-6">
    <div class="bg-gradient-to-r from-green-50 to-emerald-50 dark:from-gray-800 dark:to-gray-700 rounded-xl p-6 border border-green-100 dark:border-gray-600">
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-gray-600 dark:text-gray-400">Gerencie os banners de destaque da sua associação.</p>
            </div>
            <a href="{{ route('associacao.banners.create') }}" 
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div class="bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-200 dark:border-gray-700 hover:shadow-lg transition-shadow">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-gray-600 dark:text-gray-400">Total de Banners</p>
                    <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $banners->total() }}</p>
                </div>
                <div class="w-12 h-12 bg-blue-100 dark:bg-blue-900/20 rounded-lg flex items-center justify-center">
                    <i data-lucide="image" class="w-6 h-6 text-blue-600 dark:text-blue-400"></i>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-200 dark:border-gray-700 hover:shadow-lg transition-shadow">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-gray-600 dark:text-gray-400">Banners Ativos</p>
                    <p class="text-2xl font-bold text-green-600">{{ $banners->where('status', 'active')->count() }}</p>
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-200 dark:border-gray-700 hover:shadow-lg transition-shadow">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-gray-600 dark:text-gray-400">Banners Inativos</p>
                    <p class="text-2xl font-bold text-red-600">{{ $banners->where('status', 'inactive')->count() }}</p>
                </div>
    ...
    <div class="space-y-4">
            ...
            <div class="p-6">
                <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                    <div class="flex items-center space-x-4">
                        <img class="w-24 h-16 rounded-lg object-cover border border-gray-200 dark:border-gray-700" 
                             src="{{ $banner->image_url }}" 
                             alt="{{ $banner->name }}">
                        <div class="flex-1 min-w-0">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white truncate">{{ $banner->name }}</h3>
                            <p class="text-sm text-gray-600 dark:text-gray-400 flex items-center space-x-1 mt-1">
                                <i data-lucide="link" class="w-4 h-4"></i>
                                <span class="truncate">{{ $banner->link ?? 'Nenhum link' }}</span>
                            </p>
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium mt-2
                                {{ $banner->status === 'active' ? 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400' : 
                                   'bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-400' }}">
                                <i data-lucide="{{ $banner->status === 'active' ? 'check-circle' : 'x-circle' }}" class="w-3 h-3 mr-1"></i>
                    ...
                    <div class="flex items-center space-x-2">
                        <a href="{{ route('associacao.banners.edit', $banner) }}" 
                        ...
                        <button onclick="confirmDelete('{{ $banner->id }}', '{{ $banner->name }}')"
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-12 text-center">
            ...
            <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">Nenhum banner encontrado</h3>
            <p class="text-gray-500 dark:text-gray-400 mb-6">Comece criando seu primeiro banner para a associação.</p>
            <a href="{{ route('associacao.banners.create') }}" 
    ...
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-white dark:bg-gray-800 rounded-xl max-w-md w-full p-6 transform transition-all">
            ...
            <p class="text-gray-600 dark:text-gray-400 text-center mb-6">
                Tem certeza que deseja excluir o banner <span id="deleteBannerName" class="font-medium"></span>? 
...
<script>
    function confirmDelete(bannerId, bannerName) {
        ...
        document.getElementById('deleteForm').action = `{{ route('associacao.banners.destroy', '') }}/${bannerId}`;
